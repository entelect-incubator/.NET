...
public sealed class GetProductsQueryHandler(DatabaseContext databaseContext, IMapper mapper) : IQueryHandler<GetProductsQuery, Result<IEnumerable<ProductDTO>>>
{
    public async Task<Result<IEnumerable<ProductDTO>>> HandleAsync(GetProductsQuery request, CancellationToken cancellationToken)
    {
        ...
        {
            return Result<IEnumerable<ProductDTO>>.Failure("Product search criteria is required");
        }
