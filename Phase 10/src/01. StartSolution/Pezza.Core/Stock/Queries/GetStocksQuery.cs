...
public sealed class GetStocksQueryHandler(DatabaseContext databaseContext, IMapper mapper) : IQueryHandler<GetStocksQuery, Result<IEnumerable<PizzaModel>>>
{
    public async Task<Result<IEnumerable<PizzaModel>>> HandleAsync(GetStocksQuery request, CancellationToken cancellationToken)
    {
        ...
        {
            return Result<IEnumerable<PizzaModel>>.Failure("Stock search criteria is required");
        }
