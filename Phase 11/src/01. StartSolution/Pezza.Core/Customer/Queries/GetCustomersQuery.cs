...
public sealed class GetCustomersQueryHandler(DatabaseContext databaseContext, IMapper mapper)
    : IQueryHandler<GetCustomersQuery, Result<IEnumerable<CustomerDTO>>>
{
    public async Task<Result<IEnumerable<CustomerDTO>>> HandleAsync(GetCustomersQuery request, CancellationToken cancellationToken)
    {
        ...
        {
            return Result<IEnumerable<CustomerDTO>>.Failure("Customer search data is required");
        }
