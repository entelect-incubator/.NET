...
public sealed class CustomerController : ApiController
{
	...
	public async Task<ActionResult> GetCustomer(int id, CancellationToken cancellationToken)
		=> ResponseHelper.ResponseOutcome(await new GetCustomer(id).ExecuteAsync(this.Dispatcher, cancellationToken), this);
	...
	public async Task<ActionResult> Search(CancellationToken cancellationToken)
		=> ResponseHelper.ResponseOutcome(await new GetCustomers().ExecuteAsync(this.Dispatcher, cancellationToken), this);
	...
	[ProducesResponseType(typeof(Result<CustomerModel>), 200)]
	[ProducesResponseType(typeof(ErrorResult), 400)]
	public async Task<ActionResult<CustomerModel>> Create(CreateCustomerModel model, CancellationToken cancellationToken)
		=> ResponseHelper.ResponseOutcome(await this.Dispatcher.Send(new CreateCustomer(model), cancellationToken), this);
	...
	public async Task<ActionResult> Update(int id, UpdateCustomerModel model, CancellationToken cancellationToken)
		=> ResponseHelper.ResponseOutcome(await this.Dispatcher.Send(new UpdateCustomer(id, model), cancellationToken), this);
	...
	public async Task<ActionResult> Delete(int id, CancellationToken cancellationToken)
		=> ResponseHelper.ResponseOutcome(await this.Dispatcher.Send(new DeleteCustomer(id), cancellationToken), this);
}
