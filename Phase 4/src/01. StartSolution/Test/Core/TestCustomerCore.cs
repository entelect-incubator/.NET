...
public class TestCustomerCore : QueryTestBase
{
	...
	public async Task Init()
	{
		...
		var sutCreate = new CreateCustomerHandler(this.Context);
		var resultCreate = await sutCreate.Handle(new CreateCustomer(
	...
	{
		var sutGet = new GetCustomerHandler(this.Context);
		var resultGet = await sutGet.Handle(new GetCustomer(this.model.Id), CancellationToken.None);
	...
	{
		var sutGetAll = new GetCustomersHandler(this.Context);
		var resultGetAll = await sutGetAll.Handle(new GetCustomers(), CancellationToken.None);
	...
	{
		var sutUpdate = new UpdateCustomerHandler(this.Context);
		var resultUpdate = await sutUpdate.Handle(new UpdateCustomer(this.model.Id,
	...
	{
		var sutDelete = new DeleteCustomerHandler(this.Context);
		var outcomeDelete = await sutDelete.Handle(new DeleteCustomer(this.model.Id), CancellationToken.None);
