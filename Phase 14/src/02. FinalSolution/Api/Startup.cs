...
public class Startup
{
    ...
    public void ConfigureServices(IServiceCollection services)
    {
        ...
        var deliveryServiceUrl = this.Configuration["DeliveryService:BaseUrl"] ?? "http://localhost:8081";
        var loggerFactory = LoggerFactory.Create(b => b.AddConsole());
        var deliveryLogger = loggerFactory.CreateLogger<Core.Delivery.DeliveryService>();

        services.AddHttpClient<Core.Delivery.IDeliveryService, Core.Delivery.DeliveryService>(client =>
        {
            client.BaseAddress = new Uri(deliveryServiceUrl);
            client.Timeout = TimeSpan.FromSeconds(30);
        })
        .AddPolicyHandler(Core.Delivery.DeliveryPolicyExtensions.GetRetryPolicy(deliveryLogger))
        .AddPolicyHandler(Core.Delivery.DeliveryPolicyExtensions.GetCircuitBreakerPolicy(deliveryLogger));
    }
