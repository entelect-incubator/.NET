...
param(
    ...
    [switch]$DryRun = $false,
    [switch]$Backup = $true,
    [string]$BackupFolder = "$ProjectRoot/../Backup-$(Get-Date -Format 'yyyyMMdd-HHmmss')"
)
...
Write-Host "║ LiteBus Migration Script - MediatR → LiteBus Converter            ║" -ForegroundColor Cyan
Write-Host "╚═══════════════════════════════════════════════════════════════════╝" -ForegroundColor Cyan
...
function Create-Backup {
    ...
    if ($Backup) {
        ...
        $relativePath = $SourcePath.Replace($ProjectRoot, "")
...
function Update-CommandInterfaces {
    Log-Action "Updating command interfaces (IRequest → ICommand)..." "INFO"
...
function Update-QueryInterfaces {
    Log-Action "Updating query interfaces (IRequest -> IQuery)..." "INFO"
...
function Update-HandlerInterfaces {
    Log-Action "Updating handler interfaces (IRequestHandler -> ICommandHandler/IQueryHandler)..." "INFO"
    ...
    foreach ($file in $handlerFiles) {
        ...
        if ($content -match 'namespace (.*?);') {
            ...
            if ($currentNamespace -notmatch "Handlers") {
                ...
                Log-Action "Updated namespace: $currentNamespace → $newNamespace" "DEBUG"
                $moved++
            }
        }
    }
...
function Update-ControllerDependencies {
    Log-Action "Updating controller DI (IMediator → ICommandMediator, IQueryMediator)..." "INFO"
    ...
    foreach ($file in $controllerFiles) {
        ...
        if ($content -match "IMediator mediator") {
            ...
            $newContent = $newContent -replace 'mediator\.Send\((.*?CreatePizza.*?)\)', 'cmdMediator.SendAsync($1)'
            $newContent = $newContent -replace 'mediator\.Send\((.*?CreateCustomer.*?)\)', 'cmdMediator.SendAsync($1)'
            $newContent = $newContent -replace 'mediator\.Send\((.*?CreateOrder.*?)\)', 'cmdMediator.SendAsync($1)'
            $newContent = $newContent -replace 'mediator\.Send\((.*?Update.*?)\)', 'cmdMediator.SendAsync($1)'
            $newContent = $newContent -replace 'mediator\.Send\((.*?Delete.*?)\)', 'cmdMediator.SendAsync($1)'
            $newContent = $newContent -replace 'mediator\.Send\((.*?GetPizzas.*?)\)', 'qryMediator.QueryAsync($1)'
            $newContent = $newContent -replace 'mediator\.Send\((.*?GetCustomers.*?)\)', 'qryMediator.QueryAsync($1)'
            $newContent = $newContent -replace 'mediator\.Send\((.*?Get.*?)\)', 'qryMediator.QueryAsync($1)'
            
            # Fallback for remaining Send calls that might be queries
            $newContent = $newContent -replace 'mediator\.Send\(', 'cmdMediator.SendAsync(' # Assume command by default
            
            # Update using statements
            $newContent = $newContent -replace 'using MediatR;', 'using LiteBus.Commands.Abstractions;'
            $newContent = $newContent -replace 'using LiteBus.Commands.Abstractions;', 'using LiteBus.Commands.Abstractions;`nusing LiteBus.Queries.Abstractions;'
...
function Generate-MigrationReport {
    ...
    $report = @"
# MediatR → LiteBus Migration Report
...
- Updated mediator.Send() to cmdMediator.SendAsync() for commands
- Updated mediator.Send() to qryMediator.QueryAsync() for queries
...
try {
    Write-Host "`n▶ Step 1: Creating LiteBus folder structure..." -ForegroundColor Cyan
    Create-LiteBusStructure
    
    Write-Host "`n▶ Step 2: Updating command interfaces..." -ForegroundColor Cyan
    Update-CommandInterfaces
    
    Write-Host "`n▶ Step 3: Updating query interfaces..." -ForegroundColor Cyan
    Update-QueryInterfaces
    
    Write-Host "`n▶ Step 4: Updating handler interfaces..." -ForegroundColor Cyan
    Update-HandlerInterfaces
    
    Write-Host "`n▶ Step 5: Organizing handler namespaces..." -ForegroundColor Cyan
    Update-HandlerNamespaces
    
    Write-Host "`n▶ Step 6: Updating controller dependencies..." -ForegroundColor Cyan
    Update-ControllerDependencies
    
    Write-Host "`n▶ Step 7: Generating migration report..." -ForegroundColor Cyan
